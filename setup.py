...
def generate_vllm_cuda_extension():
    ...
    if _is_cuda():
        ...
        VLLM_BUILD_ARGS.build_ext = build_ext
        VLLM_BUILD_ARGS.distclass = BinaryDistribution
    ...
    VLLM_BUILD_ARGS.version = get_vllm_version()
    VLLM_BUILD_ARGS.requirements = get_requirements()
    VLLM_BUILD_ARGS.ext_modules = ext_modules
...
if VLLM_TARGET_DEVICE == "cuda" or VLLM_TARGET_DEVICE == "neuron" or VLLM_TARGET_DEVICE == "rocm":
    generate_vllm_cuda_extension()

setuptools.setup(
    name="vllm",
    version=VLLM_BUILD_ARGS.version,
    author="vLLM Team",
    license="Apache 2.0",
    ...
    packages=setuptools.find_packages(exclude=("benchmarks", "csrc", "docs",
                                               "examples", "tests")),
    python_requires=">=3.8",
    install_requires=VLLM_BUILD_ARGS.requirements,
    ext_modules=VLLM_BUILD_ARGS.ext_modules,
    cmdclass={"build_ext": VLLM_BUILD_ARGS.build_ext}
    if not _is_neuron() else {},
    distclass=VLLM_BUILD_ARGS.distclass,
